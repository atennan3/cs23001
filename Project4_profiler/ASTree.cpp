...
void AST::mainHeader(std::vector<std::string>& profileNames) {
  ...
  for (std::vector<std::string>::iterator nameItr = profileNames.begin();
    ...
    //change profile name to filename _ => .
    ...
    std::string addedText = "profile " + *nameItr + "(\"" + filename + "\");\n";

    //add an extra line after end because I want
    if (nameItr == --profileNames.end()) {
      addedText = addedText + "\n";
    }

    //create new pointer to a subtree
    AST * declareChildPtr = new AST(token, addedText);
...
void AST::fileHeader(const std::string& profileName) {
  ...
  std::string addedText = "extern profile " + profileName + ";\n";

  //Create new pointer to a subtree
  AST * declarationChildPtr = new AST(token, addedText);
